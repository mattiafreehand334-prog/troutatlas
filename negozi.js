...
fetch("negozi.json")
  ...
  .then(shops => {
    ...
    shops.forEach((shop, i) => {
      ...
      const partnership = shop.partner && shop.partner.official
        ? '<br><span style="font-size:11px;color:#2563eb;font-weight:700">🤝 PARTNERSHIP</span>'
        : '';
      marker.bindPopup(`<strong>${shop.name}</strong><br><span style="font-size:12px;color:#666">${shop.type}</span>${partnership}`);
      marker.on("click", () => {
    ...
    list.innerHTML = shops.map((s, i) => {
      const isPartner = s.partner && s.partner.official;
      const profileLink = s.partner && s.partner.profileEnabled
        ? `<a class="btn-sm btn-sm-blue" href="shop.html?id=${encodeURIComponent(s.id)}">Scopri</a>`
        : '';
      return `
        <div class="shop-card${isPartner ? ' shop-card--partner' : ''}" id="shop-${i}">
          <div class="shop-icon">🏪</div>
          <div class="shop-body">
            <div class="shop-title-row">
              <h3>${s.name}</h3>
              ${isPartner ? '<span class="shop-partnership-badge">Partnership</span>' : ''}
            </div>
            <div class="shop-type">${s.type}</div>
            <div class="shop-addr">📍 ${s.address}</div>
            <div class="shop-spec">🎣 ${s.speciality}</div>
            <div class="shop-btns">
              <a class="btn-sm btn-sm-green" href="#" onclick="navigateTo(${s.coordinates.lat},${s.coordinates.lng});return false;">🧭 Portami qui</a>
